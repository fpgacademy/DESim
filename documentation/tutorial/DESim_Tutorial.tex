...
\renewcommand{\versnum}{18.1, 19.1} %version number quartus/AMP
\newif\ifquesta
...
\item Compiling and simulating {\it DESim} demonstration projects
...
\vspace{-1em}
\begin{itemize}
	\item A good working knowledge of the \hdlName~hardware description language (HDL)
...
briefly in Section~\ref{sec:advanced}.
...
{\it adder}, which is provided as a {\it demo} project that is installed along with the 
...
navigate to \ifquesta{either \texttt{demos/modelsim/\demos} (if you are using {\it ModelSim}) or 
\texttt{demos/questa/\demos} (if you are using {\it Questa}),}
\else{the {\it DESim} \texttt{demos} folder, }\fi click on
the \texttt{adder} folder, and then click on the \texttt{Select Folder} button to open the
project.

\begin{figure}[h]
	\begin{center}
        \setlength{\fboxsep}{0pt}
        \fbox{\includegraphics[width = \textwidth]{figures/open_adder_windows.png}}
	\end{center}
		  \caption{Opening the {\it adder} project.}
	\label{fig:open_windows}
\end{figure}

The {\it adder} project folder, contains the 
folders named \texttt{sim} and \texttt{tb}, as well as the files called {\it adder.\hdlFileExt}, 
...
The {\it adder.\hdlFileExt} file, shown in Figure~\ref{fig:addern}, is the \hdlName~code that will 
...
{\it DESim} GUI, we {\it instantiate} the {\it adder} \hdlModuleName~in another \hdlName~\hdlModuleName~
...
The {\it adder} \hdlModuleName~is instantiated in {\it top.\hdlFileExt} by the statement
\ifthenelse{\value{hdlNum}=0}
	{\lstinputlisting[language=Verilog,numbers=none,firstline=12,lastline=12]{../../demos/\demos/adder/top.\hdlFileExt}}
...
To compile the {\it adder} project in the {\it DESim} GUI, click the \texttt{Compile Testbench}
command. This command executes a script called {\it run\_compile}, which is 
found in the \texttt{sim} folder of the {\it adder} project. This script comprises
...
of the compiler, \texttt{vlog ../tb/*.v}, compiles the Verilog source code in the {\it adder}
project's \texttt{tb} folder. This folder holds the {\it testbench} for the project, which is
described shortly. If the SystemVerilog or Verilog version of the {\it adder} project was opened, the 
second call to {\it vlog} compiles the source-code of the {\it adder} project, which includes the files 
{\it adder.v} and {\it top.v} or {\it adder.sv} and {\it top.sv}. If instead the SystemVerilog or VHDL 
versions of the {\it adder} project was opened, the call to {\it vcom} compiles the source-code of the 
{\it adder} project, which includes the files {\it adder.vhd} and {\it top.vhd}. Any messages produced 
...
all of the code in the testbench is needed for the {\it adder} project.  The first line 
...
\lstinputlisting[language=Verilog,numbers=none,firstline=29,lastline=30]{../../demos/verilog/adder/tb/tb.v}
...
%	\lstinputlisting[language=Verilog,numbers=none,firstline=8]{../../demos/addern/tb/tb.v}
    ...
    46}]{../../demos/verilog/adder/tb/tb.v}
\end{minipage}
...
{\it adder} project. This script runs {\it vsim}, the {\it ModelSim/Questa} Verilog
...
{\it adder} project, except that there is a \hdlName~source-code file named {\it Counter.\hdlFileExt}. 
...
project is similar to the one used for the {\it adder} project, shown in Figure~\ref{fig:tb}. The
...
ones used for the {\it adder} and {\it counter} projects.
...
\item vsim error: Load of "./simfpga.vpi" failed: File not found
\item vsim error: Load of "./simfpga.vpi" failed: Bad DLL format
